...
public class InputStreamOverrideReadCheck extends IssuableSubscriptionVisitor {
  ...
  private static final MethodMatcher READ_BYTES_INT_INT = MethodMatcher.create().ofType(TypeCriteria.anyType()).names("read").addParametersMatcher("byte[]", "int", "int");
  private static final MethodMatcher READ_INT = MethodMatcher.create().ofType(TypeCriteria.anyType()).names("read").addParametersMatcher("int");
