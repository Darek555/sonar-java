...
public class PreferStreamAnyMatchCheck extends AbstractMethodDetection {
  ...
  private static final MethodMatchers FIND_METHODS = MethodMatchers.or(
    STREAM_TYPES.stream()
      .map(type -> MethodMatchers.create().ofTypes(type).names("findFirst", "findAny").addWithoutParametersMatcher().build())
      .collect(Collectors.toList()));

  private static final MethodMatchers MAP_METHODS = MethodMatchers.or(
    STREAM_TYPES.stream()
      .map(type -> MethodMatchers.create().ofTypes(type).names("map").addParametersMatcher("java.util.function.Function").build())
      .collect(Collectors.toList()));

  private static final MethodMatchers FILTER_METHODS = MethodMatchers.or(
    STREAM_TYPES.stream()
      .map(type -> MethodMatchers.create().ofTypes(type).names("filter").withAnyParameters().build())
      .collect(Collectors.toList()));

  private static final MethodMatcher BOOLEAN_VALUE = MethodMatcher.create().ofTypes("java.lang.Boolean")
    .names("booleanValue").addWithoutParametersMatcher();
  ...
  protected MethodMatchers getMethodInvocationMatchers() {
    ...
    Stream.of("java.util.Optional", "java.util.OptionalInt", "java.util.OptionalLong", "java.util.OptionalDouble")
      .map(type -> MethodMatcher.create().ofTypes(type).names("isPresent").addWithoutParametersMatcher())
      .forEach(matchers::add);
    STREAM_TYPES.stream()
      .map(type -> MethodMatcher.create().ofTypes(type).names("anyMatch").addParameter("java.util.function.Predicate"))
      .forEach(matchers::add);
