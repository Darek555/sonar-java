...
public class LDAPDeserializationCheck extends AbstractMethodDetection {
  ...
  protected MethodMatchers getMethodInvocationMatchers() {
    return MethodMatchers.or(
      MethodMatcher.create().ofType(TypeCriteria.subtypeOf(CLASS_NAME)).names(CONSTRUCTOR_NAME).withAnyParameters(),
      MethodMatcher.create().ofType(TypeCriteria.subtypeOf(CLASS_NAME)).names("setReturningObjFlag").addParametersMatcher("boolean"));
  }
  @Override
