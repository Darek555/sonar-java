...
public class PredictableSeedCheck extends AbstractMethodDetection {
  ...
  private static final MethodMatcher GET_BYTES = MethodMatcher.create().ofTypes("java.lang.String").names("getBytes").withAnyParameters();

  @Override
  protected MethodMatchers getMethodInvocationMatchers() {
    return MethodMatchers.or(
      MethodMatcher.create().ofTypes(JAVA_SECURITY_SECURE_RANDOM).names("<init>").addParametersMatcher("byte[]"),
      MethodMatcher.create().ofTypes(JAVA_SECURITY_SECURE_RANDOM).names("setSeed").addParametersMatcher("byte[]"),
      MethodMatcher.create().ofTypes(JAVA_SECURITY_SECURE_RANDOM).names("setSeed").addParametersMatcher("long")
      );
  }
