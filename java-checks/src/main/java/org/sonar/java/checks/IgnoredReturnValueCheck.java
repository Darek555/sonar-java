...
public class IgnoredReturnValueCheck extends IssuableSubscriptionVisitor {
  ...
  private static final MethodMatchers EXCLUDED = MethodMatchers.or(
    MethodMatcher.create().ofTypes("java.lang.Character").names("toChars").addParametersMatcher("int", "char[]", "int"),
    MethodMatcher.create().ofTypes(JAVA_LANG_STRING).names("intern").addWithoutParametersMatcher());

  private static final MethodMatcher STRING_GET_BYTES = MethodMatcher.create().ofTypes(JAVA_LANG_STRING).names("getBytes").addParametersMatcher("java.nio.charset.Charset");
